...
Do exactly what was asked — no more, no less. Touch only what you must. Don't
improve adjacent code or refactor things that aren't broken. Remove only
imports/variables/functions YOUR changes made unused. Every changed line traces
directly to the user's request.

## Think Before Coding

State assumptions explicitly; ask when uncertain. If two valid approaches exist,
present both with tradeoffs — don't pick silently (skip for trivial tasks).
